...
set -eo pipefail
ln -s $1 $1.elf
$RUNFILES_DIR/openocd {interface_config_string} -c "transport select {transport}" {chip_config_string} -c "adapter_khz {programmer_frequency}; program $1.elf verify reset exit"
...
def _openocd_remote_execution_wrapper_impl(ctx):
    # Write firmware to memory location 0x8000000 i.e. start of executable flash
    interface_config_string = ""
    for config in ctx.attr.interface_configs:
        interface_config_string = interface_config_string + " -f " + config
    ...
    script_template = """
set -eo pipefail
source /worker/uploader_worker.sh
ln -s $1 $1.elf
$RUNFILES_DIR/openocd {interface_config_string} -c "hla_serial $HLA_SERIAL; transport select {transport}" {chip_config_string} -c "adapter_khz {programmer_frequency}; program $1.elf verify reset exit"
$RUNFILES_DIR/serial_wrapper -p $PORT -f {fail_string} -s {success_string} -b $BAUDRATE
"""
    script = ctx.actions.declare_file("%s.sh" % ctx.label.name)
...
openocd_remote_execution_wrapper = rule(
    implementation = _openocd_remote_execution_wrapper_impl,
    doc = """
Starts gdb debug server on TCP port :3333

Example:
    openocd_remote_execution_wrapper(
        name = "stm32h7",
        device_configs = [
            "target/stm32h7x_dual_bank.cfg",
        ],
